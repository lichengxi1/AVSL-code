...
class AVSLEmbedder(WithRecorder):
    ...
    def compute_embedding_at_i(self, idx, input):
        ...
        # get parameters
        conv = getattr(self, f"conv1x1_{idx}")
        # compute embeddigs
        output = conv(output)
        output = output.view(output.size(0), -1) # B * output_dim
        return output
    ...
    def _linearize(self, input):
        ...
        out = F.max_unpool2d( # inverse of maxpooling with 0 padding in non-maximum values
            *F.adaptive_max_pool2d(
                input, output_size=1, return_indices=True
            ),  # parse as input, indices
            kernel_size=(H, W)
        ) * H * W  # why add H * W：scaling to keep the overall brightness of image
        return out 
